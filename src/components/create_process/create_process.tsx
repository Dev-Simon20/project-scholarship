...
import { RequirementWithoutId } from "@/types/requiremet";
import { crearteScholarShip } from "@/actions/scholarships/create";
...
export function CreateScholarshipProcessForm() {
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      const invalidRequirements = requirements.filter(
         (req) =>
            !req.title || !req.sub_title || req.allowed_file_types.length === 0
      );
      ...
      try {
         ...
         //   console.log('Documentos iguales');
         // }
         const dataProceso: ScholarshipProcessWithoutCounts = {
            title: processData.title,
            sub_title: processData.sub_title,
            comment: processData.comment,
            steps_count: processData.steps_count,
            open_date: processData.open_date,
            close_date: processData.close_date,
            created_by: processData.created_by,
         };
         const Requirement: RequirementWithoutId[] =
            processData.requirements.map((r) => ({
               title: r.title,
               sub_title: r.sub_title,
               step_number: r.step_number,
               max_zise_mb: r.max_size_mb,
               url_file_example: r.url_file_expamle || null,
               document_type: r.document_type,
               allowed_file_types: r.allowed_file_types,
            }));

         const res = await crearteScholarShip(dataProceso, Requirement);
         console.log('respuesta en el front',res);
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                     ...
                     <Input
                        ...
                        value={stepsCount}
                        onChange={(e) =>
                           setStepsCount(Number.parseInt(e.target.value) || 1)
                        }
                        min="1"
                        readOnly
                     />
